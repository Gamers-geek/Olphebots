...
module.exports = {
    data: new SlashCommandBuilder()
        .setName(`tempban`)
        .setDescription(`Bannis de manière temporaire un membre du serveur.`)
        ...
        .addStringOption(option => {
            option.setName(`temps`)
                .setDescription(`La durée du bannissement.`)
                .setRequired(true);
            return option;
        })
        .addBooleanOption(option => {
            option.setName(`messages`)
            .setDescription(`Décide de si les messages de la personnes seront supprimés ou non`)
            .setRequired(true)
            return option
        })
        .addStringOption(option => {
    ...
    async execute(interaction, client) {
        ...
        if(!banTime) return interaction.reply({ content: `Veuillez préciser une durée valide pour le bannissement`, ephemeral: true });
        ...
        await target.send({ content: `Vous avez été banni temporairement du serveur **${interaction.guild.name}** par ${Formatters.inlineCode(interaction.user.tag)} pour la raison suivante : **${reason}**\n**Date de débannissement :** <t:${Date.now() + ms(time)}:d>` });
        await target.ban({ reason: reason, days: deleteMsg });
        interaction.reply({ content: `${Formatters.inlineCode(target.user.tag)} a été banni du serveur jusqu'au <t:${Date.now() + ms(time)}:d>.`, ephemeral: false });

        setTimeout(() => {
            interaction.guild.bans.remove(target.id);
        })
    }
}
